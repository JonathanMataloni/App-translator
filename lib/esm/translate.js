 ...
 */
export const translate = (originalString, capitalize) => {
    _checkInit();
    const { language, options: { caseSensitive, logs, autoCapitalize }, } = window.appTranslator;
    const dictionary = getDictionary(language);
    if (typeof originalString == 'number')
        return originalString.toString();
    if (!dictionary) {
        if (logs)
            console.warn(`Cannot find a dictionary for "${language}" in the provided collection`);
