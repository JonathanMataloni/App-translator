...
exports.tryUseBrowserLanguage = function () {
    ...
    var bcp47tag = navigator.language.toLowerCase().split('-');
    var foundDictionary = collection.find(function (dictionary) {
        return dictionary.bcp47.toLowerCase() === bcp47tag[0] ||
            dictionary.bcp47.toLowerCase() === bcp47tag[0] + "-" + bcp47tag[1];
    });
    if (!foundDictionary) {
        if (logs)
            console.warn("Browser language \"" + bcp47tag[0] + "-" + bcp47tag[1] + "\" is not available in the provided dictionaries collection.");
        return null;
    }
